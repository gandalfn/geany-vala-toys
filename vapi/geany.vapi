...
namespace Geany {
    ...
    [CCode (cprefix = "GEANY_BC_", has_type_id = false)]
    public enum BuildCmdEntries
    {
        LABEL,              /* *< The menu item label, _ marks mnemonic */
        COMMAND,            /* *< The command to run. */
        WORKING_DIR,        /* *< The directory to run in */
        CMDENTRIES_COUNT    /* *< Count of entries */
    }
    [Compact, CCode (cname = "GeanyBuildCommand")]
    public class BuildCommand
    {
        public string label;                   /* *< Menu item label */
        public string command;                 /* *< Command to run */
        public string working_dir;             /* *< working directory */
        public bool   exists;                  /* *< If the entries have valid values. */
        public bool   changed;                 /* *< Save on exit if @c changed, remove if not @c exist. */
        public bool   old;                     /* *< Converted from old format. */
    }
    [Compact]
    public class BuildDestination
    {
        ...
        string []        fileregexstr;
        [CCode (array_length = false)]
        string[]         nonfileregexstr;
    }
    [Compact, SimpleType, CCode (cname = "BuildTableData", free_function = "")]
    public class BuildTableData
    {
    }
    [CCode (cprefix = "GBF_", has_type_id = false)]
    public enum BuildFixedMenuItems
    {
        NEXT_ERROR,
        PREV_ERROR,
        COMMANDS,
    ...
    [CCode (cprefix = "GEANY_BCS_", has_type_id = false)]
    public enum BuildSource
    {
        DEF,        /* *< Default values. */
        FT,         /* *< System filetype values. */
        HOME_FT,    /* *< Filetypes in ~/.config/geany/filedefs */
        PREF,       /* *< Preferences file ~/.config/geany/geany.conf */
        PROJ_FT,    /* *< Project file filetype command */
        PROJ,       /* *< Project file if open. */
        COUNT       /* *< Count of sources. */
    }
    [CCode (cprefix = "GEANY_GBO_", has_type_id = false)]
    public enum BuildType
    {
        COMPILE,      /* *< default compile file */
        BUILD,        /* *< default build file */
        MAKE_ALL,     /* *< default make */
        CUSTOM,       /* *< default make user specified target */
        MAKE_OBJECT,  /* *< default make object, make %e.o */
        EXEC,         /* *< default execute ./%e */
        COUNT;        /* *< count of how many */

        [CCode (cname = "GBO_TO_GBG")]
        public BuildGroup to_build_group ();

        [CCode (cname = "GBO_TO_CMD")]
        public uint to_cmd ();
    }
    [CCode (cprefix = "build_", lower_case_cprefix = "build_")]
    namespace Build
    {
        ...
        public static Gtk.Widget commands_table(Document? doc, BuildSource dst, out BuildTableData data, Filetype? ft);
        public static void read_project(Filetype? ft, BuildTableData build_properties);
        public static void free_fields(BuildTableData data);

        /* build response decode assistance function */
        public static bool parse_make_dir(string str, [CCode (array_length = false)]string[] prefix);

        /* build menu functions */
        public static void menu_update(Document doc);
        public static void toolbutton_build_clicked(Gtk.Action action, void* user_data);
        public static void remove_menu_item(BuildSource src, BuildGroup grp, int cmd);
        public static BuildCommand get_menu_item(BuildSource src, BuildGroup grp, uint cmd);
        public static string get_current_menu_item(BuildGroup grp, uint cmd, BuildCmdEntries field);
        public static void set_menu_item(BuildSource src, BuildGroup grp, uint cmd, BuildCmdEntries field, string val);
        public static void activate_menu_item(BuildGroup grp, uint cmd);
        public static BuildMenuItems get_menu_items(int filetype_idx);

        /* load and store menu configuration */
        public static void load_menu(GLib.KeyFile config, BuildSource dst, void* ptr);
        public static void save_menu(GLib.KeyFile *config, void* ptr, BuildSource src);
        public static void set_group_count(BuildGroup grp, int count);
        public static uint get_group_count(BuildGroup grp);
        public static string[] get_regex(BuildGroup grp, Filetype ft, uint from);
    }
    /* reviewed */
    [Compact]
    public class Data {
    ...
    namespace Keybindings {
        ...
        [CCode (cname = "keybindings_set_item")]
        public void set_item (KeyGroup group, size_t key_id, KeyCallback? cb,
                              uint key, Gdk.ModifierType mod, string kf_name,
                              string label, Gtk.Widget? menu_item = null);
    }
    /* reviewed */
    [Compact, CCode (has_type_id = false, has_destroy_function = false)]
    ...
    namespace MessageWindow {
        [CCode (cname = "MessageWindowTabNum", cprefix = "MSG_", has_type_id = false)]
        public enum TabID {
            STATUS,
            COMPILER,
            MESSAGE,
        ...
        [CCode (cname = "MsgColors", cprefix = "COLOR_", has_type_id = false)]
        public enum Color {
            RED,
            DARK_RED,
            BLACK,
    ...
    public class Plugin {
        ...
        public unowned KeyGroup set_key_group (string section_name, size_t count, KeyGroupCallback? cb = null);
    ...
    /* reviewed */
    [CCode (cprefix = "vte_", lower_case_cprefix = "vte_")]
    namespace Vte {
        public bool         send_cmd (string cmd);
        public string       get_working_directory ();
        public void         cwd (string filename, bool force);
        public void         send_selection_to_vte ();
